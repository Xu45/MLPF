...
"""
Train_only_mlp_12axis.py

MLP-only training script for 12-axis signals with 5-dimensional regression output.
This script trains a single MLP model using all-angle data.
"""
...
if __name__ == "__main__":
    ...
    for region in range(1):
        ...
        base_dir = os.path.join(
            "train_12axis_4regions", task, "init_train", "Only_MLP"
        )
        os.makedirs(base_dir, exist_ok=True)

        root_dir = "truedata/real/dataset_regions1"

        # ---------------------------------------------------------------------
        # Load CSV Data
        # ---------------------------------------------------------------------

        data_in = pd.read_csv(f"{root_dir}/xhd_in_all_angle360.csv")
        data_out = pd.read_csv(f"{root_dir}/xhd_out_all_angle360.csv")
        ...
        joblib.dump(scaler_inputs, f"{base_dir}/scaler_inputs_all.pkl")
        joblib.dump(scaler_targets, f"{base_dir}/scaler_targets_all.pkl")
        ...
        plt.suptitle("MLP property of Region all")
        plt.tight_layout()
        plt.savefig(f"{base_dir}/mlp_result_all.png", bbox_inches="tight")
        plt.close()
